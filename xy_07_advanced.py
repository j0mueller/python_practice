...
# BTW: did you notice the use of `range` in `test_non_generator` and `xrange` in `test_generator`? # NOTE: `range` replaces `xrange` in Python 3.
...
# `range` would return back an array with 1000000 values for us to use `xrange` would generate 1000000 values for us as we request / iterate over those items
